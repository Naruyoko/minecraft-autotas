...
import minecraft_autotas.genetic.AbstractGeneticAlgorithmConcurrent;
...
public class GeneticAlgorithmConcurrentTest1 {
  ...
  static class GeneticAlgorithmConcurrent extends AbstractGeneticAlgorithmConcurrent<Individual> {
    ...
    public GeneticAlgorithmConcurrent(int populationSize, int maxGeneration, int inputLength, float mutationChance,
    ...
    public int randomParentIndex() {
      ...
      while (true) {
        ...
        float probability = chooseProbabilityOffset
            / (chooseProbabilityOffset + (float)(bestScore - lastFitnesses[choice]));
  ...
  public static void main(String[] args) {
    try {
      ...
      IXZMoveEntityHandler<? super StoneXZPlayer> moveEntityHandler = new XZMoveEntityHandlerFromCollidableSimpleGround(
          ground);
      GeneticAlgorithmConcurrent GeneticAlgorithmConcurrent = new GeneticAlgorithmConcurrent(populationSize,
          maxGeneration, inputLength, mutationChance, chooseProbabilityOffset, startingState, moveEntityHandler);
      GeneticAlgorithmConcurrent.init();
      long lastNanoTime = System.nanoTime();
      while (GeneticAlgorithmConcurrent.getGeneration() <= maxGeneration) {
        GeneticAlgorithmConcurrent.stepGeneration();
        if (GeneticAlgorithmConcurrent.getBestIndividualIndex() >= 0)
          logger.info(String.format("New best! %f : %s", GeneticAlgorithmConcurrent.getBestScore(),
              Arrays.toString(Arrays.copyOfRange(GeneticAlgorithmConcurrent.getBestIndividual().inputs, 0,
                  GeneticAlgorithmConcurrent.getEffectiveInputLength()))));
        long nanoTime = System.nanoTime();
        logger.info(String.format("Generation %d took %d ns. Best: %f", GeneticAlgorithmConcurrent.getGeneration() - 1,
            nanoTime - lastNanoTime, GeneticAlgorithmConcurrent.getBestScore()));
        lastNanoTime = nanoTime;
      }
      Individual bestIndividual = GeneticAlgorithmConcurrent.getBestIndividual();
      ...
      for (int t = -1; t < inputLength; t++) {
        if (t >= 0)
          GeneticAlgorithmConcurrent.stepPlayer(player, bestIndividual.inputs, t);
        writer1.write(String.format("%3d %5s %-22s %-10s %-22s %-22s %-22s %-22s", t + 1,
            t >= 0 ? bestIndividual.inputs[t] : "-",
            Utility.padSignDouble(GeneticAlgorithmConcurrent.immediateFitness(player)),
